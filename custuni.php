...
<body>
	...
	<h1 style="text-align: center;color: #596771;">Crawled Data of CUST University Faculty</h1>
	...
	foreach($html->find("div#yw0 table.cvlist") as $e)
	{	
		...
		//echo $e->find("p strong")->plaintext."--<br>";
			 ...
			 //echo "<tr><td>";
			 // $val1 = $e->prev_sibling().""; //name
			  //$val1 = $e->plaintext().""; //name
			 //array_push($arr1,$val1);
			 //echo "</td></tr>";
			 //echo $e->plaintext."<br>"; //name
			 $Links = $e->find('tr')[0]->find('td')[1]->find('a')[0];
			//echo $Links->plaintext."<br>"; //name
			//echo $Links->href."<br>"; //name
			if(strpos($Links->href, "page"))
			{
        echo $linksall = "https://www.cust.edu.pk".$Links->href. '<br>';
				array_push($arrlinks,$linksall);
			}
	}
	
	/* foreach($arrlinks as $pages)
	//for($i=0;$i=1;$i++)
	{
		
		//$pages = $arrlinks[$i];
	
		$html_pages = file_get_html($pages);
		
		foreach($html->find("dd.email") as $e)
		{
			echo $e->plaintext."<br>";
		}
